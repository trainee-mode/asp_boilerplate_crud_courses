...
namespace test123.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly test123DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = test123Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(test123DbContext context)
        {
            _context = context;
        }
